...
export function SourcesPage() {
  ...
  const [qrStatusText, setQrStatusText] = useState("正在获取 115 官方二维码...");
  ...
  async function refreshQr(selectedApp = appType) {
    ...
    setQrStatusText("正在请求 115 官方二维码...");
    try {
      ...
      toast.error("获取 115 官方二维码失败，请检查网络后重试");
      setQrStatusText("获取失败，请点击刷新");
    } finally {
  ...
  const disconnectMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("已退出 115 账号并清除本地凭证");
      void qc.invalidateQueries({ queryKey: ["sources"] });
      void qc.invalidateQueries({ queryKey: ["overview"] });
      void qc.invalidateQueries({ queryKey: ["115"] });
      void refreshQr(appType);
  ...
  return (
    <div className="space-y-10">
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="font-mono text-[11px] tracking-[0.22em] text-accent">PRODUCTION UPSTREAM INTEGRATION</p>
          <h1 className="mt-2 font-display text-4xl tracking-tight">115 网盘接入中心</h1>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted">
            正式生产对接 115 网盘。支持{" "}
            <span className="font-medium text-fg">苹果设备（iPhone / iPad / Mac）官方扫码登录</span> 与{" "}
            <span className="font-medium text-fg">Cookie 直连</span>。真实提取您的 115 视频素材并推送至多视图 AI 抽帧索引。
          </p>
        </div>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-line bg-surface p-3 shadow-soft">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <Badge tone="ok">{connectedUser.vipLevel || "VIP 会员"}</Badge>
              </div>
      ...
      <section className="rounded-2xl border border-border bg-surface p-6 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-medium">115 官方认证方式</h2>
          </div>
        ...
        <div className="mt-6">
                ...
                <div className="relative flex aspect-square w-52 items-center justify-center overflow-hidden rounded-xl border border-border bg-white p-2 shadow-inner">
                  {qrSession?.qrcode ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center text-zinc-500">
                      ...
                      <span className="mt-2 text-[10px]">获取官方二维码中...</span>
                    </div>
                  )}
                  ...
                  {qrStatusCode === 2 && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 p-3 text-white">
                      ...
                      <p className="mt-2 text-xs font-medium">115 登录成功</p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-muted">
                      115 对不同客户端形态下发的 Token 与会话策略略有不同。推荐使用{" "}
                      <span className="text-fg font-medium">Apple iOS (iPhone)</span> 或{" "}
                      <span className="text-fg font-medium">iPad</span> 官方客户端扫码登录。
                    </p>
                  </div>
                  ...
                  <div className="rounded-xl border border-border bg-elevated/40 p-4 text-xs leading-relaxed text-muted">
                    <p className="font-medium text-fg">📱 苹果设备真实扫码指引：</p>
                    <ol className="mt-2 list-inside list-decimal space-y-1.5 text-subtle">
                      <li>在 iPhone / iPad / Mac 上打开 115 App；</li>
                      <li>点击右上角「+」或「扫一扫」扫描左侧二维码；</li>
                      <li>在设备上点击「确认登录」，系统将自动完成官方鉴权并下发凭证。</li>
                    </ol>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "cookie" && (
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-medium">115 真实 Cookie 直连</h3>
                <p className="mt-1 text-xs text-muted">
                  从 115.com 浏览器端（F12 Network 控制台）复制完整的 Cookie 字符串（需包含 <code>UID</code>, <code>CID</code>, <code>SEID</code>）。
                </p>
              </div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="请粘贴您的真实 115 Cookie 字符串，例如：UID=...; CID=...; SEID=...; KID=..."
      ...
      <section className="space-y-4 rounded-2xl border border-border bg-surface p-6 shadow-soft">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted">
                {connectedUser
                  ? `已连接 ${connectedUser.userName} 的云端网盘`
                  : "请先通过上方扫码或 Cookie 连接 115 账号"}
              </p>
            </div>
          </div>
        ...
        {connectedUser ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-14 text-center">
            <Video className="h-10 w-10 text-subtle" />
            <h3 className="mt-3 text-sm font-medium text-fg">尚未连接 115 网盘账号</h3>
            <p className="mt-1 max-w-sm text-xs text-muted">
              请使用上方的「苹果设备扫码」或「Cookie 直连」登录您的 115 账号，系统将即时读取并同步您的真实视频素材。
            </p>
          </div>
        )}
      </section>
